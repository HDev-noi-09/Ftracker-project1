# PERSONAL FINANCE TRACKER  

## Description
Built a simple Personal Finance Tracker web app  using HTML , CSS , Node.js , Express.js ,Postman(external use) and  MongoDB Atlas for data storage. 
This project helps any user to keep a track over weekly expenses accoring to their core needs.
User can perform different operations with the app and maintain a record of thier expenses. 

## Features
- 1.User can add or insert the expenses he/she wants ,into a tabular format to have a list of expenses through a clean UI.

- 2.A user can also update the amount previously entered if thier is any need for alteration of records.
  Moreover user can also delete any expense , which is no longer needed to be tracked , according to. 

- 3.Lastly ,user can see AI-like insights of weekly expense and a category breakdown of most and least spendings over various categories.

## Working 
A very simple working where CRUD requests are being sent by frontend -UI(HTML and CSS) and JS -through fetch.

And are being accepted and processed by backend by a live server and data fetching, insertion and manipulation is done in cloud version of MongoDB(atlas).

